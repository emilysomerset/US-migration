...
```{r model-comparisons, echo = FALSE, warning = FALSE,cache=FALSE}
load("../Model results/finalmodel_spline.RData")
# Compare models 

## Check convergence of RW model 
d1 <- summary(modcs)$summary[,c("mean", "n_eff", "Rhat","sd","se_mean","2.5%","97.5%")]
totrace1 <- rownames(d1 %>% as.data.frame() %>% arrange(desc(Rhat)))[1]
rstan::traceplot(modcs, pars=totrace1,inc_warmup = TRUE)
rstan::traceplot(modcs, pars = "sigma1", inc_warmup = FALSE)
rstan::traceplot(modcs, pars = "sigma2", inc_warmup = FALSE)

## Compare parameters
# pars = c("alpha","tau","gamma","sigma1","sigma2")
# print(modrw, pars = c(pars))
# print(modrw_RI, pars = c(pars,"sigma3"))

## Goodness of fit 
yp1_p <-summary(modcs)$summary[,c("mean", "2.5%","97.5%")][grep("y_p",rownames(d1),fixed= TRUE),1]
yp1_upper <-summary(modcs)$summary[,c("mean", "2.5%","97.5%")][grep("y_p",rownames(d1),fixed= TRUE),3]
yp1_lower <- summary(modcs)$summary[,c("mean", "2.5%","97.5%")][grep("y_p",rownames(d1),fixed= TRUE),2]

data.frame(y = stan_data$y, 
           y_p = yp1_p, 
           yp_upper = yp1_upper, 
           yp_lower = yp1_lower) %>% 
  mutate(ind2 = y<= yp_upper & yp_lower <= y) %>% 
  summarise(mm = mean(ind2))
...
ggsave(filename = "../Paper/Plots/in_mig_plot_CS.pdf",
       plot=in_mig_plot, 
       device = "pdf",
       dpi = 300,
       height = 4, 
...
plots <- split(tmp_results, tmp_results$newpuma)

lapply(plots, function(dd){
dd %>% 
  filter(year<2020) %>% 
  ggplot(aes(age, rate))+
  geom_point()+ 
  ...
  geom_line(aes(age, est_rates, col = "expected + delta(a,r) + delta(t,r)"))+
...
### Estimated migration curve over time for New York PUMAS

```{r, echo = FALSE, warning=FALSE, message=FALSE, fig.height=6.5, fig.width=7}

plots <- split(tmp_results, tmp_results$newpuma)

set.seed(2)
plots <- plots[sample(1:length(plots),10)]
# plots <- plots[1:6]
...
lapply(plots, function(dd){
  ...
  geom_line(aes(year, est_rates, col = "fitted", group= age_gp)) +
  geom_ribbon(aes(ymin = fitted_rate_lower, ymax = fitted_rate_upper, col = "fitted", fill ="fitted", group = age_gp), alpha = 0.2, show.legend = FALSE)+ 
  geom_point(aes(col = "data", group = age_gp))+
  scale_color_manual(name = "", values = c("fitted" = "red", "data" = "black")) +
  theme_bw()+ 
  ggtitle(paste0("Region ", dd$newpuma[1])) 
})
