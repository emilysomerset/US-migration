...
parameters {
  ...
  matrix[A,R] delta3[T]; //matrix of deviations
...
transformed parameters{
  ...
  for (i in 1:N){
      ...
      log_rate[i] = fixed_eff[i] + delta1[age_vec[i],region_vec[i]] + delta3[time_vec[i]][age_vec[i],region_vec[i]];
      }
    }
...
model {
  //likelihood
    ...
    for (a in 1:A){
      ...
      if (a==1){
        target += normal_lpdf(delta1[a,1:R]|0,sigma1);
      }else{
        target += normal_lpdf(delta1[a,1:R]|delta1[(a-1),1:R],sigma1);  //random walk on age
    }}
    target += normal_lpdf(sigma3|0,1);
    
    for (a in 1:A){
      for (r in 1:R){
        target += normal_lpdf(delta3[1][a,r]|0, sigma3);
        target += normal_lpdf(delta3[2:T][a,r]|delta3[1:(T-1)][a,r], sigma3);
      }
    }
}
...
for (i in 1:(A*R)){
 pred_rate[i] = pred_fixed_eff[i]+ delta1[pred_age_vec[i],pred_region_vec[i]] + normal_rng(delta3[T][pred_age_vec[i],pred_region_vec[i]], sigma3);
 }
}
