...
modrw_RI <- stan(data = stan_data,
             file = "../Programs/models/final models/finalmodel_rw_RI.stan",
             iter=1000,
             warmup = 250,
             cores=4,
             chains = 4,
             ...
             thin=3,
             seed=1)
model_sum <- summary(modrw_RI)
interaction = FALSE
save(file="../Model results/finalmodel_rw_RI_moreits.RData", list=c("modrw_RI","model_sum", "interaction"))

```


```{r check-convergence, echo = FALSE, warning = FALSE,cache=FALSE}
load("../Model results/finalmodel_rw_RI_moreits.RData")

## Check convergence of RW and Random Intercept model 
d2 <- summary(modrw_RI)$summary[,c("mean", "n_eff", "Rhat","sd","se_mean","2.5%","97.5%")]
totrace2 <- rownames(d2 %>% as.data.frame() %>% arrange(desc(Rhat)))[1]
rstan::traceplot(modrw_RI, pars=totrace2,inc_warmup = FALSE)
rstan::traceplot(modrw_RI, pars = "sigma1", inc_warmup = FALSE)
rstan::traceplot(modrw_RI, pars = "sigma2", inc_warmup = FALSE)

## Check parameters
pars = c("alpha","tau","gamma","sigma1","sigma2")
print(modrw_RI, pars = c(pars))

## Goodness of fit 
yp2_p <-summary(modrw_RI)$summary[,c("mean", "2.5%","97.5%")][grep("y_p",rownames(d2),fixed= TRUE),1]
yp2_upper <-summary(modrw_RI)$summary[,c("mean", "2.5%","97.5%")][grep("y_p",rownames(d2),fixed= TRUE),3]
yp2_lower <- summary(modrw_RI)$summary[,c("mean", "2.5%","97.5%")][grep("y_p",rownames(d2),fixed= TRUE),2]
...
d2 <- d[grep("tau|alpha|sigma1|sigma2|delta1|delta2|delta3|gamma",rownames(d)),]
# d2 %>% as.data.frame() %>% arrange(desc(Rhat)) %>% head(10)
totrace <- rownames(d2 %>% as.data.frame() %>% arrange(desc(Rhat)))[1]
d %>% as.data.frame() %>% arrange(desc(Rhat))
rstan::traceplot(modrw_RI, pars=totrace,inc_warmup = TRUE)
rstan::traceplot(modrw_RI, pars='gamma[24]',inc_warmup = TRUE)
